...
StateLight GravitationalEntity::statePonto(const float& t, const StateLight& estado) const {
	//const v3d raio = (estado.s - center);
	const double h2 = cross(estado.v, estado.s).norm2();
	return StateLight{ estado.v, -1.5 * h2 * estado.s / pow(estado.s.norm2(), 2.5) };
}


bool GravitationalEntity::response(v3d& rayDirection, v3d& intersection, v3d& intersectionNormal, olc::Pixel& pix) {
	const StateLight inicial{ (intersection - center) / schwarzschildRadius , rayDirection };

	//std::cout << "Novo: " << inicial.s.x << ", " << inicial.s.y << ", " << inicial.s.z << "  .  " << inicial.v.x << ", " << inicial.v.y << ", " << inicial.v.z << std::endl;
	StateLight final = RungeKuttaSolver::RungeKutta4Solverf(0.01f, 100000, inicial, (*this));
